// ============================================
// Merchant Pages - Centralized Exports
// ouyaboung Platform - Anti-gaspillage alimentaire
// ============================================

export { default as MerchantDashboardPage } from "./MerchantDashboardPage";
export { default as MerchantProductsPage } from "./MerchantProductsPage";
export { default as MerchantOrdersPage } from "./MerchantOrdersPage";
export { default as MerchantAnalyticsPage } from "./MerchantAnalyticsPage";
export { default as MerchantImpactPage } from "./MerchantImpactPage";
export { default as MerchantProfilePage } from "./MerchantProfilePage";
export { default as MerchantSettingsPage } from "./MerchantSettingsPage";
